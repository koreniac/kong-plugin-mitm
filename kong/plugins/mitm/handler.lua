...
MitmHandler.PRIORITY =  13
MitmHandler.VERSION = "0.11.0"
...
local function _log(conf,get_raw_body)
    print("_log/conf.http_endpoint = ", conf.http_endpoint)
    print("_log/getRawBody = ", get_raw_body)
    local obj_before_encode = basic_serializer.serialize(ngx)
    -- modify obj_befor_encode with getRawBody
    obj_before_encode["request"]["body"] = get_raw_body
...
function MitmHandler:log(conf)
    ...
    if (flush_case == "FAILED" and response_code >= 400) then
      ...
      _log(ctx.conf, ctx.request_body)
