local redirect_to = require "kong.plugins.respond-redirect.http_post"
...
local Mitm = BasePlugin:extend()

function Mitm:new()
  Mitm.super.new(self, "Mitm")
end

function Mitm:log(conf)
  redirect_to.replaceHeader(conf)
end

function Mitm:header_filter(conf)
  redirect_to.replaceHeader(conf)
end

function Mitm:body_filter(conf)
